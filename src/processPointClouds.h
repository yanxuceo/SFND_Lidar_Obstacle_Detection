...
template <typename PointT> class ProcessPointClouds {
  ...
  struct KdTree {
    ...
    void searchHelper3D(PointT target, Node *node, int depth, float distanceTol,
                        std::vector<int> &ids) {
      if (node != NULL) {
...
private:
  ...
  void clusterHelper3D(int indice, typename pcl::PointCloud<PointT>::Ptr points,
    ...
    std::vector<int> nearest =
        tree->search3D(points->points[indice], distanceTol);
